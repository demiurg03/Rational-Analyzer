...
DatabaseInterface::DatabaseInterface(Database *db) : QObject(nullptr), _db(db)
{
...
std::optional<Product> DatabaseInterface::getProductById(const int id){
    ...
    auto result = getProductByPredicate([id](const Product &product){
...
QVector<Product> DatabaseInterface::getProductByPredicate(std::function<bool (const Product &)> fn, bool stopFirst){
